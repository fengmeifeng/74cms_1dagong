...
//添加一个插入操作，专门插入发布的展会----------------------------------------------------------
function inserttablejobfair($tablename, $insertsqlarr) {
	global $db;
	$insertkeysql = $insertvaluesql = $comma = '';
	...
	$state = $db->query("INSERT INTO $tablename ($insertkeysql) VALUES ($insertvaluesql)");
...
function subsiteinfo(&$_CFG)
{
	...
	else
	{
		...
		if (array_key_exists($host,$_SUBSITE))
		{
			...
			$subsite['dizhi']?$_CFG['subsite_dizhi']=$subsite['dizhi']:'';
			$subsite['phone']?$_CFG['subsite_phone']=$subsite['phone']:'';
		}
	}
}
...
$w = date('w',time());
echo str_replace(" {$w} ", $day_wak[$w], date('Y-m-d w H:i:s A',time()));
