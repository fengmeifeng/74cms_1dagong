...
function get_apply_ck($offset,$perpage,$get_sql= '')
{
	...
	$limit=" LIMIT ".intval($offset).','.intval($perpage);
	//$select="i.*,j.subsite_id,j.jobs_name,j.addtime,j.companyname,j.company_addtime,j.district_cn,j.wage_cn,j.deadline,j.refreshtime,j.click";
	$select="i.*,j.companyname,j.addtime,j.refreshtime,j.look_num,j.uid";
	$sql="SELECT {$select} from ".table('company_apply_ck')." AS i {$get_sql} ORDER BY did DESC {$limit}";
	$result = $db->query($sql);
	while($row = $db->fetch_array($result))
	{
		/*if (empty($row['companyname']))	
		...
		$row['company_url']=url_rewrite('QS_companyshow',array('id'=>$row['uid']));
		//$row['jobs_url']=url_rewrite('QS_jobsshow',array('id'=>$row['jobs_id']),true,$row['subsite_id']);
		//简历title
		$resume_info = $db->getone("select * from ".table('resume')." where id=".intval($row['resume_id']));
		$row['title'] = $resume_info['title'];
...
function get_favorites($offset,$perpage,$get_sql= '')
{
	...
	$row['companyname'] = cut_str($row['companyname'],8,0,"...");
	$row['company_url']=url_rewrite('QS_companyshow',array('id'=>$row['company_id']));
	$row['jobs_url']=url_rewrite('QS_jobsshow',array('id'=>$row['jobs_id']),true,$row['subsite_id']);
	$row_arr[] = $row;
	}
...
function app_get_hymq($id)
{
	...
	$sql = "select * from ".table('shenhe_jobs')." WHERE id IN ({$id}) ";
	return $db->getall($sql);
}
function check_hymq_apply($jobs_id,$resume_id,$p_uid)
{
	global $db;
	$sql = "select id from ".table('bm_hymq')." WHERE personal_uid = '".intval($p_uid)."' AND aid='".intval($jobs_id)."'  AND resume_id='".intval($resume_id)."' LIMIT 1";
	return $db->getone($sql);
}
//--
...
function check_course_yuyue($mobile,$courseid)
{
	global $db;
	$sql = "select did from ".table('personal_course_apply')." WHERE  mobile='".$mobile."' AND course_id='".intval($courseid)."' LIMIT 1";
	return $db->getone($sql);
}
