...
function get_gifts($uid)
{
	global $db;
	$uid=intval($uid);
	$sql = "select m.*,g.*,t.* from ".table('members_gifts')." as m left join ".table('gifts')." as g on m.account=g.account left join ".table("gifts_type")." as t  on t.t_id=m.giftstid where uid='{$uid}' ORDER BY m.id desc";
	$list=$db->getall($sql);
	return $list;
}
