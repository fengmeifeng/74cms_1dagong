...
if (!empty($aset['category']) || !empty($aset['subclass']) || !empty($aset['jobcategory']))
{
	...
	if (!empty($aset['jobcategory']))
	{
					...
					foreach($arr as $sid)
					{
						...
						if (intval($cat[1])===0)
						{
						$dsql.= " OR j.category =".intval($cat[0]);
						}
						else
						{
						$xsql.= " OR j.subclass =".intval($cat[1]);
						}
					}
	...
	else
	{
					if (!empty($aset['category']))
					{
						if (strpos($aset['category'],"-"))
						{
							...
							{
								$joinwheresql.=" AND j.category IN  ({$sqlin}) ";
							}
						}
						else
						{
							$joinwheresql.=" AND  j.category=".intval($aset['category']);
						}
					}
					if (!empty($aset['subclass']))
					{
						if (strpos($aset['subclass'],"-"))
						{
							...
							if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
							{
								$joinwheresql.=" AND j.subclass IN  ({$sqlin}) ";
							}
						}
						else
						{
						$joinwheresql.=" AND j.subclass=".intval($aset['subclass']);
						}
					}
	...
	$joinsql="  left  JOIN  ".table('resume_jobs')." AS j ON  r.id=j.pid ";
}
...
if (isset($aset['trade']) && $aset['trade']<>'')
{
	...
	{
		$resumetable=table('resume');
		$wheresql.=" AND r.trade=".intval($aset['trade'])." ";
...
$wheresql=" WHERE ".ltrim(ltrim($wheresql),'AND');
	...
	$list = $id = array();
	$idresult = $db->query("SELECT r.id FROM {$resumetable}  AS r".$joinsql.$wheresql.$orderbysql.$limit);
