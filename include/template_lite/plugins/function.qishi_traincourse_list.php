<?php
function tpl_function_qishi_traincourse_list($params, &$smarty)
{
global $db,$_CFG;
...
if (!empty($uidarr))
{
	...
	{		
		...
		//$companyarray[$row['uid']]['district_cn']=cut_str($row['district_cn'],5,0,'..');
		...
		$companyarray[$row['uid']]['jobs'][$row['id']]['jobs_url']=url_rewrite($aset['jobsshow'],array('id'=>$row['id']),true,$row['subsite_id']);
