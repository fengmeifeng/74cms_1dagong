...
//----培训不区分分站--fff
if($aset['alias'] =='QS_trainfocus' || $aset['alias'] =='QS_traincentreimg'){
	
}else{///-------其他区分分站
	
	if ($_CFG['subsite']=="1" && $_CFG['subsite_filter_ad']=="1" && intval($_CFG['subsite_id'])>0 )
	{
		$wheresql.=" AND subsite_id in (".intval($_CFG['subsite_id']).",999)";
	}
	else{///----
	///---全站-全站首页轮番广告显示
	if($aset['alias'] =='QS_indexfocus'){
		$wheresql.=" AND subsite_id in (0,999) ";
	}
	//------ffff	
	}	
}
...
//echo $wheresql.$orderbysql.$limit;exit;
