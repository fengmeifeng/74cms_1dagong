...
{		
	...
	$ydqy=$db->getall("select z.*,c.id as cid from vip_zhanhui as z left join ".table('company_profile')." as c  on z.qid=c.uid  where z.zid='".$row['id']."'and z.qid <>0  order by z.add_time desc limit 0,12");		//已预订过的企业
	$kehu=0;
	foreach($ydqy as $key=>$car){
     //在遍历的过程中为 car添加一个新的属性比如生产日期
	 $kehu++;
    $ydqy[$key]['kehu'] = $kehu;
	}
	//echo "<pre>";print_r($ydqy);exit;
	//var_dump($ydqy);exit;
	$row['ydqy']=$ydqy;
	///-----------只有发布职位的企业显示---ffffff
	/*
