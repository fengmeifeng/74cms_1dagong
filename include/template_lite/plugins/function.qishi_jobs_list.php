...
if (isset($aset['displayorder']))
{
		...
		{
		$orderbysql=" ORDER BY a.addtime {$arr[1]} , a.refreshtime {$arr[1]}";
		$jobstable=table('jobs');
		//echo $orderbysql;exit;
		}
		//----fffff
		else
		{
...
if (isset($aset['page']))
{
	...
	$result = $db->query("SELECT *,a.refreshtime,a.id,b.audit,b.logo FROM ".table('jobs')." a LEFT JOIN qs_company_profile b on a.uid=b.uid".$wheresql.$orderbysql);	
