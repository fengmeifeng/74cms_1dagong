...
if($_CFG['operation_train_mode']=='1'){
	$wheresql.="  AND audit=1  AND display=1 AND add_mode=1 AND deadline >=".time();
}elseif($_CFG['operation_train_mode']=='2'){
	$wheresql.="  AND audit=1  AND display=1 AND add_mode=2 AND deadline >=".time()." AND setmeal_id>0 AND (setmeal_deadline>{$timenow} OR setmeal_deadline=0)";
}
...
if (isset($aset['page']))
{
	...
	while($row = $db->fetch_array($result))
	{
		...
		$row['course_url']=url_rewrite($aset['courseshow'],array('id'=>$row['id']));
