...
function refresh_jobs($id,$uid)
{
	...
	{
		if (!$db->query("update  ".table('hunter_profile')."  SET refreshtime='{$time}' WHERE uid='{$uid}' LIMIT 1 ")) return false;
		if (!$db->query("update  ".table('hunter_jobs')."  SET refreshtime='{$time}' WHERE uid='{$uid}' and utype='{$utype}'")) return false;
		return true;
	}
	return false;
}
