...
elseif($act == 'company_contact')
{
	...
	if ($id>0)
	{
		...
		$val=$db->getone($sql);
			...
			//----ff---判断电话是否公开，--公开显示图片
				if($val['telephone_show']=='1'){
					$html.='</div><div class="telimg"><a href="tel:'.$val["telephone"].'"><img src="images/23.jpg"  border="0"/></a></div>';//---公开
				}else{
				$html.='</div>';//---不公开	
				}
			//----ff---结束
...
{   //echo "21121";exit;
	$id=intval($_GET['id']);
		...
		elseif($_CFG['showresumecontact']=='2')
		{
			...
			{
			$show=false;
			//$html="<div style='color:red;' align='center'>不能查看,仅该会员投递您发布的职位时才可以查看</div>";
			$html='<div class="resume_show_box" id=""><div class="title"><h2>联系方式</h2></div><div class="txt">';
			$html.='企业会员请<a href="wap_login.php">[登录]</a>后查看联系方式<br />没有帐号？<a href="wap_user_reg.php">[免费注册]</a>';
			$html.='</div><div class="telimg"></div></div>';
			}
		...
		if ($show)
		{
			$tb1=$db->getone("select fullname,telephone,email,residence_cn from ".table('resume')." WHERE  id='{$id}'  LIMIT 1");
			$val=$tb1;
