//---fff
//注册地区
function OpenCity_zhuce(objid,input_cn,input,inputs,titstr,district_cns)
{
	$(objid).unbind().click(function()
	{
			...
			if ($("#selectbox").html()==null)
			{
				...
				$("#selectbox #listboxb .li").click( function () {
					...
					//---ff
					var sheng = $("#selectbox #listboxb .li .h").attr('title');//alert(sheng);
					//---		
					...
					$(this).find(".t2").addClass("h");		
					$(input_cn).html(sheng+'/'+$(this).attr('title'));
					//$(input_cn).val($(this).attr('title'));
					$(inputs).val($(this).attr('id'));
					$(district_cns).val(sheng+'/'+$(this).attr('title'));
					$("#selectbox").hide();
					//
				});
...
function MakeLiB(arr)
{
			...
			var htmlstr='';
				...
				{
				 var v=arr[x].split(",");
				htmlstr+="<div class=\"li\"  title=\""+v[1]+"\" id=\""+v[0]+"\"><div class=\"t1\">"+v[1]+"</div><div class=\"t2\" title=\""+v[1]+"\"></div><div class=\"clear\"></div></div>";
				}
			return htmlstr; 
}
//生成小类
