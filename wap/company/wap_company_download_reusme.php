...
elseif ($act == 'index')
{
	...
	$sql="select a.*,r.title,r.fullname,r.display_name,r.education_cn,r.birthdate,r.experience_cn,r.residence_cn from ".table("company_down_resume")." as a left join ".table("resume")." as r on a.resume_id=r.id  $wheresql order by a.down_addtime desc ".$limit;
	$row=$db->getall($sql);
...
elseif($act=="ajax_download_resume")
{
	...
	$wheresql=" WHERE a.company_uid='{$_SESSION['uid']}' ";
	$row=$db->getall("select a.*,r.title,r.fullname,r.display_name,r.education_cn,r.birthdate,r.experience_cn,r.residence_cn from ".table("company_down_resume")." as a left join ".table("resume")." as r on a.resume_id=r.id  $wheresql order by a.down_addtime desc limit $offset,$rows");
...
elseif($act=="ajax_download_resume_add")
{
	...
	else
	{
		if ($_CFG['down_resume_limit']=="1")
		{
			...
			if (empty($user_jobs))
			{
				exit("您没有发布职位或审核未通过导致无法下载简历");
			}
			else
			{	
				$sql = "select 1 from ".table('personal_jobs_apply')." WHERE company_uid = {$_SESSION['uid']} AND resume_id='{$resume_id}' LIMIT 1";
				$info=$db->getone($sql);
				if(!empty($info)){
					exit("ok");
				}else{
					exit('不能查看,仅该会员投递您发布的职位时才可以查看');
				}
				...
				elseif($_CFG['operation_mode']=="1")
				{
					...
					if  ($mypoints<$points)
					{
						...
						{
							exit('你的服务已到期或超出服务条数。您可以等网页版申请服务');
						}
						else
						{
	...
	{	
			if ($resume['talent_']=='2')
			{
					...
					if($pms_notice=='1'){
						...
						$resume_url=url_rewrite('QS_resume',array('id'=>$id),true,$resume['subsite_id']);
