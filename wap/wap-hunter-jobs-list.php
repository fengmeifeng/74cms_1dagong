...
$wheresql=" WHERE ".ltrim(ltrim($wheresql),'AND');
	...
	$page = empty($_GET['page'])?1:intval($_GET['page']);
	if($page<1) $page = 1;
	$theurl = "wap-hunter-jobs-list.php?sdistrict=".$sdistrict."&amp;subclass=".$subclass."&amp;key=".$key;
	...
	if (!empty($id))
	{
		...
		$jobs = $db->getall("SELECT * FROM ".table('hunter_jobs').$wheresql.$orderbysql);
		foreach ($jobs as $key => $value) {
			$jobs[$key]['url'] = wap_url_rewrite("wap-hunter-jobs-show",array("id"=>$value['id']));
		}	
	}
	else
	{
	...
	$smarty->display("wap/wap-hunter-jobs-list.html");
?>