...
elseif($act == 'editsave')
{
	...
	$link[1]['href'] = "?act=edit&id=".$id;
 	!updatetable(table('explain'),$setsqlarr," id=".$id."")?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}
elseif($act == 'add')
{
	get_token();
...
{	
	...
	$link[1]['text'] = "返回说明页列表";
	$link[1]['href'] = '?';
	!inserttable(table('explain'),$setsqlarr)?adminmsg("添加失败！",0):adminmsg("添加成功！",2,$link);
}
	...
	if (is_array($_POST['categoryname']) && count($_POST['categoryname'])>0)
	{
		for ($i =0; $i <count($_POST['categoryname']);$i++){
			...
			{		
				...
				!inserttable(table('explain_category'),$setsqlarr)?adminmsg("添加失败！",0):"";
...
elseif($act == 'edit_category_save')
{
	...
	!updatetable(table('explain_category'),$setsqlarr," id=".$id."")?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}
