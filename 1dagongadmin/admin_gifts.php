...
if($act == 'list')
{
	...
	else
	{
		...
		///----ffff
		$be=isset($_GET['be'])?intval($_GET['be']):"";
		if ($be===0)
		{
		$wheresql=empty($wheresql)?" WHERE g.be=0":$wheresql." AND g.be=0 ";
		}
		if ($be===1)
		{
		$wheresql=empty($wheresql)?" WHERE g.be = 1":$wheresql." AND g.be = 1 ";
		$oederbysql=" order BY g.usettime  DESC";
		}
		///----ffff
		...
		{
		$wheresql=empty($wheresql)?" WHERE t.t_id= ".intval($_CFG['subsite_id']):$wheresql." AND t.t_id= ".intval($_CFG['subsite_id']);
		}
		//----
	}
...
elseif($act == 'use')
{
	...
	if ($key && $key_type>0)
	{
		...
		if     ($key_type===1)$wheresql=" WHERE g.account like '%{$key}%'";
		if     ($key_type===2)$wheresql=" WHERE m.username like '%{$key}%'";
		if     ($key_type===3)$wheresql=" WHERE g.uid like'%{$key}%'";
		if     ($key_type===4)$wheresql=" WHERE m.email like '%{$key}%'";
		if     ($key_type===5)$wheresql=" WHERE m.mobile like '%{$key}%'";
		$oederbysql="";
	}
	else
	{
