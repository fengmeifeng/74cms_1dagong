function tagso(templatedir,dir,jobslisturl,getarr,district_id)
{
	...
	opthtm+='<input name="tag" type="hidden" value="'+get[4]+'" />';
	...
	if (get[4]=='')
	{
		...
		opthtm+='<li><a href="'+href+'" id="tag-'+arr[0]+'"  class="opt">'+arr[1]+'</a></li>';
		}
	}
	else
	{
	...
	function search_location()
	{
		...
		var tag=$("#searcform input[name=tag]").val();
		var inforow=$("#searcform input[name=inforow]").val();
		var page=$("#searcform input[name=page]").val();
		$.get(dir+"plus/ajax_search_location.php", {"act":"QS_resumetag","category":category,"subclass":subclass,"district":district,"sdistrict":sdistrict,"tag":tag,"inforow":inforow,"page":page},
