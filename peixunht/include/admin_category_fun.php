...
//--------------------------------fffff培训分类开始
function get_category_train()
{
	global $db;
	$sql = "select * from ".table('category_train')." where parentid=0  order BY category_order desc,id asc";
	$result = $db->query($sql);
	while($row = $db->fetch_array($result))
	{		
		$sql = "select * from ".table('category_train')." where parentid=".$row['id']."  order BY category_order desc,id asc";
		$sub=$db->getall($sql);
		$row['sub']=$sub;
		$category[]=$row;
	}
...
function get_category_train_one($id)
{
	global $db;
	$sql = "select * from ".table('category_train')." WHERE id=".intval($id)." LIMIT 1";
	return $db->getone($sql);
}
function del_train_category($id)
{
	global $db;
	if(!is_array($id)) $id=array($id);
	$return=0;
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		if (!$db->query("Delete from ".table('category_train')." WHERE id IN (".$sqlin.") ")) return false;
		$return=$return+$db->affected_rows();
		if (!$db->query("Delete from ".table('category_train')." WHERE parentid IN (".$sqlin.") ")) return false;
		$return=$return+$db->affected_rows();
	}
	return $return;
}
//--------------------------------------------------ffffff培训分类结束
//高级职位
function get_category_hunterjobs()
{
	global $db;
	$sql = "select * from ".table('category_hunterjobs')." where parentid=0  order BY category_order desc,id asc";
	$result = $db->query($sql);
	while($row = $db->fetch_array($result))
	{		
		$sql = "select * from ".table('category_hunterjobs')." where parentid=".$row['id']."  order BY category_order desc,id asc";
		$sub=$db->getall($sql);
		$row['sub']=$sub;
		$category[]=$row;
	}
...
//高级职位
function del_hunterjobs_category($id)
{
	global $db;
	if(!is_array($id)) $id=array($id);
	$return=0;
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		if (!$db->query("Delete from ".table('category_hunterjobs')." WHERE id IN (".$sqlin.") ")) return false;
		$return=$return+$db->affected_rows();
		if (!$db->query("Delete from ".table('category_hunterjobs')." WHERE parentid IN (".$sqlin.") ")) return false;
		$return=$return+$db->affected_rows();
	}
	return $return;
}
//高级职位
function get_category_hunterjobs_one($id)
{
	global $db;
	$sql = "select * from ".table('category_hunterjobs')." WHERE id=".intval($id)." LIMIT 1";
	return $db->getone($sql);
}
