...
$act = !empty($_GET['act']) ? trim($_GET['act']) : 'train';
...
if($act == 'grouplist')
{
	...
	$smarty->display('category_px/admin_category_group.htm');
}
elseif($act == 'add_group')
{
	get_token();
	$smarty->assign('navlabel',"group");
	$smarty->display('category_px/admin_category_group_add.htm');
}
...
elseif($act == 'edit_group')
{
	...
	$smarty->display('category_px/admin_category_group_edit.htm');
}
...
elseif($act == 'show_category')
{
	...
	$smarty->display('category_px/admin_category_list.htm');
}
...
elseif($act == 'add_category')
{
	...
	$smarty->assign('navlabel',"group");
	$smarty->assign('group',get_category_group_one($_GET['alias']));
	$smarty->display('category_px/admin_category_add.htm');
}
...
{	
	...
	$smarty->display('category_px/admin_category_edit.htm');
}
...
elseif($act == 'district')
{
	...
	$smarty->display('category_px/admin_category_district.htm');
}
...
elseif($act == 'edit_district')
{
	...
	$smarty->display('category_px/admin_category_district_edit.htm');
}
...
elseif($act == 'add_district')
{
	get_token();
	$smarty->assign('navlabel',"district");
	$smarty->display('category_px/admin_category_district_add.htm');
}
/*-------------------------------------------------------------*/
//添加三级分类界面
elseif($act == 'add_sandistrict')
{
	get_token();
	if(!empty($_GET['sanpid'])){
		$sql = "select * from ".table('category_district')." where id={$_GET['pid']}";
		$data=$db->getall($sql);
		$smarty->assign('data',$data[0]);
		$smarty->assign('pid',$_GET['pid']);
		$smarty->assign('sanpid',$_GET['sanpid']);
	}
	$smarty->display('category_px/admin_category_sandistrict_add.htm');
}
//修改界面
elseif($act == 'edit_sandistrict')
{
	get_token();
	if(!empty($_GET['sanpid'])){
		$sql = "select * from ".table('category_district')." where id={$_GET['pid']}";
		$data=$db->getall($sql);
		$smarty->assign('data',$data[0]);
		$sql = "select * from ".table('category_district')." where id={$_GET['sanpid']}";
		$dat=$db->getall($sql);
		$smarty->assign('dat',$dat[0]);
	}
	$smarty->display('category_px/admin_category_sandistrict_edit.htm');
}
//三级分类入库
elseif($act == 'add_sandistrict_save')
{
	check_token();
	//新增的入库
	print_r($_POST);
...
elseif($act == 'jobs')
{
	...
	//var_dump(get_category_jobs());exit;
	$smarty->display('category_px/admin_category_jobs.htm');
}
...
elseif($act == 'edit_jobs_category')
{
	...
	$smarty->display('category_px/admin_category_jobs_edit.htm');
}
...
elseif($act == 'add_category_jobs')
{
	get_token();
	$smarty->assign('navlabel',"jobs");
	$smarty->display('category_px/admin_category_jobs_add.htm');
}
...
elseif($act == 'train')
{
	...
	get_token();
	$smarty->assign('navlabel',"train");
	$smarty->assign('district',get_category_train());
	//var_dump(get_category_jobs());exit;
	$smarty->display('category_px/admin_category_train.htm');
}
elseif($act == 'train_all_save')
{
	check_token();
	if (is_array($_POST['save_id']) && count($_POST['save_id'])>0)
	{
		for ($i =0; $i <count($_POST['save_id']);$i++){
			...
			{	
				...
				$setsqlarr['category_order']=intval($_POST['category_order'][$i]);
				$setsqlarr['content']=trim($_POST['content'][$i]);				
				!updatetable(table('category_train'),$setsqlarr," id=".intval($_POST['save_id'][$i]))?adminmsg("保存失败！",0):"";
				$num=$num+$db->affected_rows();
			}
		}
	}
	...
	if (is_array($_POST['add_pid']) && count($_POST['add_pid'])>0)
	{
		for ($i =0; $i <count($_POST['add_pid']);$i++){
			...
			{	
				...
				$setsqlarr['content']=trim($_POST['content'][$i]);	
				$setsqlarr['parentid']=intval($_POST['add_pid'][$i]);	
				!inserttable(table('category_train'),$setsqlarr)?adminmsg("保存失败！",0):"";
				$num=$num+$db->affected_rows();
			}
...
elseif($act == 'del_train_category')
{
	check_token();
	$id=$_REQUEST['id'];
	if ($num=del_train_category($id))
	{
	adminmsg("删除成功！共删除".$num."行",2);
	}
	else
...
elseif($act == 'edit_train_category')
{
	get_token();
	//echo get_token();exit;
	$smarty->assign('navlabel',"train");
	$smarty->assign('category',get_category_train_one($_GET['id']));
	$smarty->display('category_px/admin_category_train_edit.htm');
}
elseif($act == 'edit_jobs_category_save')
{
	check_token();
	//echo check_token();exit;
	$setsqlarr['categoryname']=!empty($_POST['categoryname']) ?trim($_POST['categoryname']) : adminmsg("请填写名称",1);
	$setsqlarr['category_order']=intval($_POST['category_order']);
	$setsqlarr['content']=trim($_POST['content']);
	$setsqlarr['parentid']=intval($_POST['parentid']);				
	!updatetable(table('category_train'),$setsqlarr," id=".intval($_POST['id']))?adminmsg("修改失败！",0):"";
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = '?act=train';
	makejs_classify();
	makejs_train_classify();
	adminmsg("保存成功！",2,$link);
}
elseif($act == 'add_category_train')
{
	get_token();
	$smarty->assign('navlabel',"train");
	$smarty->display('category_px/admin_category_train_add.htm');
}
elseif($act == 'add_category_train_save')
{
	check_token();
	//新增的入库
	if (is_array($_POST['categoryname']) && count($_POST['categoryname'])>0)
...
elseif($act == 'hunter_jobs')
{
	...
	$smarty->display('category_px/admin_category_hunterjobs.htm');
}
...
elseif($act == 'edit_hunterjobs_category')
{
	...
	$smarty->display('category_px/admin_category_hunterjobs_edit.htm');
}
...
elseif($act == 'add_category_hunterjobs')
{
	get_token();
	$smarty->assign('navlabel',"hunter_jobs");
	$smarty->display('category_px/admin_category_hunterjobs_add.htm');
}
...
elseif($act == 'colorlist')
{
	...
	$smarty->display('category_px/admin_color.htm');
}
elseif($act == 'add_color')
{
	get_token();
	$smarty->assign('navlabel',"color");
	$smarty->display('category_px/admin_color_add.htm');
}
...
elseif($act == 'edit_color')
{
	...
	$smarty->display('category_px/admin_color_edit.htm');
}
