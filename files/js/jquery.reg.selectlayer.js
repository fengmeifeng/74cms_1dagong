function allaround(dir){
	...
	//-----个人注册地区填充
	if($("#divCityCate_1").length > 0) {
		fillCity("#divCityCate_1"); // 地区填充
		// 恢复地区选中条件
		if($("#residence_1").val()) {
			var scityid = $("#residence").val().split(".");
			if(scityid[1] == 0) {
				var dcityid = scityid[0];
				$("#divCityCate_1 li p a").each(function() {
					if(dcityid == $(this).attr("rcoid")) {
						$(this).addClass('selectedcolor');
					}
				});
			} else {
				$("#divCityCate_1 .citycatebox .subcate a").each(function() {
		...
		/* 地区点击显示到已选 */
		$("#divCityCate_1 li p a").unbind().live('click', function(){
			$("#divCityCate_1 li p a").each(function() {
				$(this).removeClass('selectedcolor');
			});
			$(this).addClass('selectedcolor');
			...
			var checkText = $(this).attr('title');
			$("#cityText_1").html(checkText);
			$("#residence_cn").val(checkText);
			$("#residence").val(checkID);
			$("#divCityCate_1").hide();
		});
		$("#divCityCate_1 .subcate a").unbind().live('click', function() {		
			$("#divCityCate_1 .subcate a").each(function() {
				$(this).parent().prev().find('font a').removeClass('selectedcolor');
				$(this).removeClass('selectedcolor');
			});
			$(this).parent().prev().find('font a').addClass('selectedcolor');
