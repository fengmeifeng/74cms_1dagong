function allaround(dir,getstr){
	...
	showOption("#resumeducation","education",QS_education);	//学历
	// 恢复学历选中条件
	if(getstr) {
		if(getstr[3]) {
			$("#searoptions").show();
			var edustr = "";
			$("#resumeducation a").each(function() {
				$(this).removeClass('selc');
				var demoEduArray = $(this).attr('id').split('-');
				if(getstr[3] == demoEduArray[1]) {
					$(this).addClass('selc');
					edustr = $(this).html();
				}
			});
	...
	showOption("#resumexperience","experience",QS_experience); // 经验
	// 恢复经验选中条件
	if(getstr) {
		if(getstr[4]) {
			$("#searoptions").show();
			var expstr = "";
			$("#resumexperience a").each(function() {
				$(this).removeClass('selc');
				var demoExpArray = $(this).attr('id').split('-');
				if(getstr[4] == demoExpArray[1]) {
					$(this).addClass('selc');
					expstr = $(this).html();
				}
			});
	...
	if(getstr) {
		if(getstr[5]) {
			$("#searoptions").show();
			var photostr = "";
			$("#resumephoto a").each(function() {
				$(this).removeClass('selc');
				var demoPhotoArray = $(this).attr('id').split('-');
				if(getstr[5] == demoPhotoArray[1]) {
					$(this).addClass('selc');
					photostr = $(this).html();
				}
			});
			checkedstr += '<a href="javascript:;" ty="photo" class="cnt"><span>'+photostr+'</span><i class="del"></i></a>';
		}
	}
	// 恢复等级选中条件
	if(getstr) {
		if(getstr[6]) {
			$("#searoptions").show();
			var talentstr = "";
			$("#resumetalent a").each(function() {
				$(this).removeClass('selc');
				var demoTalentArray = $(this).attr('id').split('-');
				if(getstr[6] == demoTalentArray[1]) {
					$(this).addClass('selc');
					talentstr = $(this).html();
				}
			});
			checkedstr += '<a href="javascript:;" ty="talent" class="cnt"><span>'+talentstr+'</span><i class="del"></i></a>';
		}
	}
	// 恢复性别选中条件
	if(getstr) {
		if(getstr[7]) {
			$("#searoptions").show();
			var talentstr = "";
			$("#resumesex a").each(function() {
				$(this).removeClass('selc');
				var demoSexArray = $(this).attr('id').split('-');
				if(getstr[7] == demoSexArray[1]) {
					$(this).addClass('selc');
					talentstr = $(this).html();
				}
			});
			checkedstr += '<a href="javascript:;" ty="sex" class="cnt"><span>'+talentstr+'</span><i class="del"></i></a>';
		}
	}
	// 关键字显示到以选择
	...
	function search_location() {
		...
		var photo=$("#searckeybox input[name=photo]").val();
		var talent=$("#searckeybox input[name=talent]").val();
		var sex=$("#searckeybox input[name=sex]").val();
		var sort_1=$("#searckeybox input[name=sort]").val();
		var page=$("#searckeybox input[name=page]").val();
		$.get(dir+"plus/ajax_search_location.php", {"act":"QS_resumelist","key":key,"trade":trade,"jobcategory":jobcategory,"citycategory":citycategory,"education":education,"experience":experience,"photo":photo,"talent":talent,"sex":sex,"sort":sort_1,"page":page},
