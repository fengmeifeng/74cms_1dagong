...
<body>
<div align="center">
<table width="88%" border="1" cellspacing="1" style="border-collapse: collapse" bordercolorlight="#000000" bordercolordark="#000000">
  <tr>
 ...
 <td align="center" valign="middle">编号</td>
   ...
   <!-- <td align="center" valign="middle">政治面貌</td>
    ...
    <td align="center" valign="middle">录入成绩</td>
  </tr><?php
...
//考场号和座位号  
  ...
  //录入分数
 if($rs->status == 1){ 
	if($rs->ksbh != '' && $rs->kch != '' && $rs->zwh != ''){
  echo"  <td align=\"center\" valign=\"middle\"><form method=\"post\" action=\"?act=add_cj\" id=\"form_2\" name=\"form_2\"><input type=\"hidden\" name=\"id\" value=\"".$rs->id."\" />职业:<input type=\"text\" name=\"km1\" value=\"".$rs->km1."\"  id=\"km1\" size=\"4\" />综合:<input type=\"text\" name=\"km2\" value=\"".$rs->km2."\"  id=\"km2\" size=\"4\" />面试:<input type=\"text\" name=\"mscj\" value=\"".$rs->mscj."\"  id=\"mscj\" size=\"4\" /><input type=\"submit\" value=\"提交\"/></form></td>";
	}else{
		echo"  <td align=\"center\" valign=\"middle\"></td>";  
		}
...
{
	 //echo '录入成绩';exit;
	$km1=trim($_REQUEST['km1']);
	$km2=trim($_REQUEST['km2']);
	$mscj=trim($_REQUEST['mscj']);
	$id=trim($_REQUEST['id']);
	if($km1 && $km2 && $mscj !=""){
	$qsl_km="update qs_baoming set km1 = '".$km1."',km2 ='".$km2."',mscj='".$mscj."'  where id =".$id;
	//echo $qsl_id;exit;
	if($db->query($qsl_km))
	{
		echo "<script language=\"JavaScript\">\r\n";   echo " alert(\"录入成功\");\r\n";   echo " history.back();\r\n";   echo "</script>";
