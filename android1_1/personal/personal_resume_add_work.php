...
{
	$uid_source = $db->query("select uid from ".table('members')." where `username`='".$aset['username']."'");
	$uid_arr = $db->fetch_array($uid_source);
	$uid = $uid_arr['uid'];
	$pid = intval($aset['pid']);
	if($aset['act']=="add"){
		$resume_search = $db->query("select * from ".table('resume_work')." where `pid`=".$pid);
	...
	if($aset['act']=="save"){
		...
		$setsqlarr['companyprofile']=trim($aset['companyprofile']);
		$setsqlarr['jobs']=trim($aset['jobs']);
		$setsqlarr['achievements']=trim($aset['achievements']);
		...
		if (empty($setsqlarr['companyname']))
		{
			$result['result']=0;
			$result['list']=null;
			$result['errormsg']=android_iconv_utf8('企业名称不能为空！');
			$jsonencode = urldecode(json_encode($result));
			exit($jsonencode);
		}
		if (empty($setsqlarr['jobs']))
		{
			$result['result']=0;
			$result['list']=null;
			$result['errormsg']=android_iconv_utf8('职务名称不能为空！');
			$jsonencode = urldecode(json_encode($result));
			exit($jsonencode);
		}
		
		if($eid==0){
			$update_result=inserttable(table('resume_work'),$setsqlarr,1);
		}else{
			$wheresql = "`id`=".$eid;
			$update_result=updatetable(table('resume_work'),$setsqlarr,$wheresql);
		}
		...
		if ($update_result)
		{
			...
			$result['errormsg']=android_iconv_utf8('成功保存个人简历工作经历！');
			$jsonencode = urldecode(json_encode($result));
			exit($jsonencode);
		}else{
			$result['result']=0;
			$result['list']=null;
			$result['errormsg']=android_iconv_utf8('保存个人简历工作经历失败！');
			$jsonencode = urldecode(json_encode($result));
			exit($jsonencode);
		}
	}
	if($aset['act']=="delete"){
		$eid = intval($aset['eid']);
		$delete_source = $db->query("delete from ".table('resume_work')." where id=".$eid);
		if($delete_source){
			$result['result']="1";
			$result['list']=null;
