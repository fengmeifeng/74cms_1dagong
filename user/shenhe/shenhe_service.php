...
if ($act=='account')
{
	...
	$smarty->assign('title','我的账户 - 审核员中心 - '.$_CFG['site_name']);
	...
	if($_CFG['operation_shenhe_mode']=="1"){
		$smarty->display('member_shenhe/shenhe_my_account.htm');
	}elseif($_CFG['operation_shenhe_mode']=="2"){
		$smarty->display('member_shenhe/shenhe_my_account_package.htm');
	}
	
}
elseif ($act=='order_add')
{
	$smarty->assign('title','在线充值 - 审核员中心 - '.$_CFG['site_name']);
	$smarty->assign('payment',get_payment());
	$smarty->assign('points',get_user_points($_SESSION['uid']));
	$smarty->display('member_shenhe/shenhe_order_add.htm');
}
elseif ($act=='order_add_save')
{
		if (empty($hunter_profile['shenhename']))
		{
		$link[0]['text'] = "填写基本资料";
		$link[0]['href'] = 'shenhe_info.php?act=shenhe_profile';
		showmsg("请先填写您的基本资料！",1,$link);
		}
	...
	($amount<$_CFG['shenhe_payment_min'])?showmsg("单笔充值金额不能少于 ".$_CFG['shenhe_payment_min']." 元！",1):'';
	...
	$points=$amount*$_CFG['shenhe_payment_rate'];
	$order_id=add_order($_SESSION['uid'],$order['oid'],$amount,$payment_name,"充值积分:".$points,$timestamp,$points,'',3);
		if ($order_id)
			{
...
elseif ($act=='payment')
{
	...
	$smarty->assign('points',get_user_points($_SESSION['uid']));
	$smarty->assign('title','付款 - 审核员中心 - '.$_CFG['site_name']);
	...
	$smarty->display('member_shenhe/shenhe_order_pay.htm');
}
elseif ($act=='order_list')
{
	...
	$smarty->assign('title','充值记录 - 审核员中心 - '.$_CFG['site_name']);
	$smarty->assign('act',$act);
	...
	$smarty->display('member_shenhe/shenhe_order_list.htm');
}
...
elseif ($act=='setmeal_list'  && $_CFG['operation_shenhe_mode']=="2")
{
	$setmeal = get_user_setmeal($_SESSION['uid']);
	if ($setmeal['endtime']>0){
		$setmeal_endtime=sub_day($setmeal['endtime'],time());
	...
	$smarty->display('member_shenhe/shenhe_setmeal_list.htm');
}
elseif ($act=='setmeal_order_add'  && $_CFG['operation_shenhe_mode']=="2")
{
	...
	$smarty->assign('title','申请服务 - 审核员中心 - '.$_CFG['site_name']);
	$smarty->assign('payment',get_payment());
	$smarty->display('member_shenhe/shenhe_order_add_setmeal.htm');
}
elseif ($act=='setmeal_order_add_save'  && $_CFG['operation_shenhe_mode']=="2")
{
		if (empty($hunter_profile['shenhename']))
		{
		$link[0]['text'] = "填写猎头资料";
		$link[0]['href'] = 'shenhe_info.php?act=hunter_profile';
		showmsg("请先填写您的基本资料！",1,$link);
		}
	$myorder=get_user_order($_SESSION['uid'],1);
	...
	if ($setmeal && $setmeal['apply']=="1")
	{
		...
		$order_id=add_order($_SESSION['uid'],$order['oid'],$setmeal['expense'],$payment_name,"开通服务:".$setmeal['setmeal_name'],$timestamp,"",$setmeal['id'],3);
			if ($order_id)
			{
