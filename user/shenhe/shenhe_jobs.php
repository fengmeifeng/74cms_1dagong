...
if ($act=='jobs')
{
	...
	$smarty->assign('title','行业名企管理 - 审核员中心 - '.$_CFG['site_name']);
...
if ($act=='bm_hymq')
{
	...
	$page = new page(array('total'=>$total_val, 'perpage'=>$perpage));
	$offset=($page->nowindex-1)*$perpage;
	$smarty->assign('title','报名行业名企管理 - 审核员中心 - '.$_CFG['site_name']);
	$smarty->assign('act',$act);
...
elseif($act=='refresh_jobs'){
	...
	write_memberslog($_SESSION['uid'],3,8504,$_SESSION['username'],"刷新行业名企");
	write_refresh_log($_SESSION['uid'],3001);					
	showmsg("刷新行业名企成功！",2);
	$smarty->assign('concern_id',get_concern_id($uid));
	$smarty->assign('title','刷新行业名企 - 审核员中心 - '.$_CFG['site_name']);
	$smarty->display('member_shenhe/shenhe_jobs.htm');
}
elseif ($act=='jobs_perform')
{
    ...
    if ($refresh)
	{
		...
		if($_CFG['operation_shenhe_mode']=='1'){
			...
			{
			showmsg("每天最多只能刷新".$_CFG['shenhe_pointsmode_refresh_time']."次,您今天已超过最大刷新次数限制！",2);	
			}
			elseif($duringtime<=$space){
			showmsg($_CFG['shenhe_pointsmode_refresh_space']."分钟内不能重复刷新职位！",2);
			}
			else 
			{
				$points_rule=get_cache('points_rule');
				if($points_rule['shenhe_shenhejobs_refresh']['value']>0)
				{
					...
					showmsg("您的".$_CFG['shenhe_points_byname']."不足，请先充值！",0,$link);
					}
		...
		elseif($_CFG['operation_shenhe_mode']=='2'){
			...
			else
			{
				...
				{
				showmsg("每天最多只能刷新".$setmeal['shenhe_refresh_jobs_time']."次,您今天已超过最大刷新次数限制！",2);	
				}
				elseif($duringtime<=$space){
				showmsg($setmeal['shenhe_refresh_jobs_space']."分钟内不能重复刷新职位！",2);
				}
			}
		}
...
elseif ($act=='edit_jobs')
{
	...
	$smarty->assign('user',$user);
	$smarty->assign('subsite',get_subsite_list());
	$smarty->assign('title','修改职位 - 审核员中心 - '.$_CFG['site_name']);
	$smarty->assign('points_total',get_user_points($_SESSION['uid']));
	$smarty->assign('points',get_cache('points_rule'));
...
elseif ($act=='editjobs_save')
{
	...
	$setsqlarr['trade_cn']=trim($_POST['trade_cn']);
	//--
	$setsqlarr['gs_district']=!empty($_POST['gs_district'])?intval($_POST['gs_district']):showmsg('请选择所在城市！',1);
	$setsqlarr['gs_sdistrict']=intval($_POST['gs_sdistrict']);
	$setsqlarr['gs_district_cn']=trim($_POST['gs_district_cn']);
	//--
	...
	//-----ffff
	if (!updatetable(table('shenhe_jobs'), $setsqlarr," id='{$id}' AND uid='{$_SESSION['uid']}' ")) showmsg("保存失败！",0);
	if ($_CFG['operation_shenhe_mode']=='1')
	{
		...
		write_memberslog($_SESSION['uid'],3,9201,$_SESSION['username'],"修改职位：<strong>{$setsqlarr['jobs_name']}</strong>，({$operator}{$points_rule['shenhe_shenhejobs_edit']['value']})，(剩余:{$user_points})");
		}
