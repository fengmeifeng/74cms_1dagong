...
require_once(QISHI_ROOT_PATH.'include/fun_shenhe.php');
	...
	showmsg('您访问的页面需要 审核员员 登录！',1,$link);
	}
	...
	$shenhe_profile=get_shenhe($_SESSION['uid']);
	if (!empty($shenhe_profile))
	{
		$shenhe_profile = array_map("addslashes",$shenhe_profile);
		$smarty->assign('shenhe_url',url_rewrite('QS_shenheshow',array('id'=>$shenhe_profile['id'])));
	}	
	$smarty->assign('userindexurl','shenhe_index.php');
