...
elseif ($act=='apply_ck_list')
{
	...
	$offset=($currenpage-1)*$perpage;
	$smarty->assign('act',$act);
	$smarty->assign('title','我申请的联系方式 - 企业会员中心 - '.$_CFG['site_name']);
...
elseif ($act=='apply_ck_del')
{
	$yid =!empty($_REQUEST['y_id'])?$_REQUEST['y_id']:showmsg("你没有选择项目！",1);
	$delete =$_POST['delete'];
	$delete?(!del_apply_ck($yid,$_SESSION['uid'])?showmsg("删除失败！",0):showmsg("删除成功！",2)):'';
}
//--ff
...
elseif($act == 'del_my_attention'){
	$yid =!empty($_REQUEST['y_id'])?$_REQUEST['y_id']:showmsg("你没有选择项目！",1);
	$delete =$_REQUEST['del'];
	$delete?(!del_my_attention($yid,$_SESSION['uid'])?showmsg("删除失败！",0):showmsg("删除成功！",2)):'';
}
unset($smarty);
?>