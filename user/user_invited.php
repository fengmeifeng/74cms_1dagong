...
if ($act=="invited")
{	
	//企业邀请求职者面试给 求职者 发送短信开始 By Z
	$query="select telephone from qs_resume where id=".$id;
	$row=$db->getone($query);
	$username=$row['telephone'];
	require_once(QISHI_ROOT_PATH.'include/msg_invited.php');
	//企业邀请求职者面试给 求职者 发送短信结束 By Z		
	if ($_CFG['operation_mode']=="2")
	{
		if ($resume['talent']=='2')
		{	
...
$("#but_invited").click(function() 
{
		...
		else
		{
			...
			$("#ajax_download_r").attr("disabled","disabled");
			 ...
			 if(pms_notice) pms_notice=1;else pms_notice=0;
			 // echo $_CFG['website_dir'] 
			$.get("/user/user_invited.php", {"jobs_id": $("#jobs_id").val(),"id":id,"notes":$("#notes").val(),"pms_notice":pms_notice,"time":tsTimeStamp,"act":"invited_save"},
 			function (data,textStatus)
			 {
				if (data=="ok")
				{
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tableall" id="ajax_invited_table">
   ...
   <!-- <tr>
    ...
    <tr>
		<td></td>
		<td>
			<a href="<?php echo get_member_url(1,true)?>company_recruitment.php?act=interview_list" target="_blank">查看我发起的面试邀请</a>&nbsp;&nbsp;&nbsp;<a href="<?php echo get_member_url(1)?>" >进入会员中心</a>
		</td>
    </tr>
</table>
<table id="notice" width="100%" border="0" style="border-top:1px #CCCCCC dotted;background-color: #EEEEEE; line-height: 230%;padding: 15px; margin-top: 10px; ">
