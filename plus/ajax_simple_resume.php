...
{	
	...
	$smarty->assign('subsite_id',$_GET['subsite_id']);
...
{	
	...
	$setsqlarr['district_cn']=trim($_POST['district_cn']);
	$setsqlarr['sdistrict_cn']=trim($_POST['sdistrict_cn']);
	$setsqlarr['detailed']=trim($_POST['detailed'])!=""?trim($_POST['detailed']):showmsg("您没有填写个人简介！",1);
	$setsqlarr['refreshtime']=time();
...
elseif($act == "get_simple_tel"){
	...
	$tel = $db->getone($sql);
	exit("联系方式：".$tel['tel']." ".$tel['uname']);
}
elseif($act == "get_simple_detailed"){
	$id=intval($_GET['id']);
	$sql = "select detailed from ".table('simple_resume')." where id=".$id;
	$detailed = $db->getone($sql);
	exit("自我描述：".$detailed['detailed'].'<a href="javascript:void(0);" class="hidden_detailed" id="'.$id.'">[收起]</a>');
}
elseif($act == "hidden_simple_detailed"){
	...
	exit("自我描述：".$detailed['detailed'].'<a href="javascript:void(0);" class="show_detailed" id="'.$id.'">[展开]</a>');
}
