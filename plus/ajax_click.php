...
elseif($act == 'course_click')
{
	...
	if ($id>0)
	{
		...
		//$sql="update ".table('course')." set click=click+7 WHERE id='{$id}'  LIMIT 1";
		$db->query($sql);
...
//--
elseif($act == 'shenhejobs_click')
{
	$id=intval($_GET['id']);
	if ($id>0)
	{
		$sql="update ".table('shenhe_jobs')." set click=click+1 WHERE id='{$id}'  LIMIT 1";
		$db->query($sql);
		$sql = "select click from ".table('shenhe_jobs')." where id='{$id}'  LIMIT 1";
		$val=$db->getone($sql);
		exit($val['click']);
	}
}
