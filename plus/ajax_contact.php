		...
		if($show==false){
			...
			elseif($_CFG['showresumecontact']=='2')
			{
				if ($_SESSION['uid'] && $_SESSION['username'] && $_SESSION['utype']=='1')
				{
					...
					//----剩余查看次数
					$sql_info = "select ck_num,ck_end_time from ".table('company_profile')." WHERE uid = {$_SESSION['uid']}";
					$company_info=$db->getone($sql_info);
					//if($company_info['ck_num'] >0 && $company_info['ck_end_time'] >time()){$show=true;}
					//---------剩余查看次数------
					//----投递简历才能查看
					$sql = "select 1 from ".table('personal_jobs_apply')." WHERE company_uid = {$_SESSION['uid']} AND resume_id='{$id}' LIMIT 1";
					$info=$db->getone($sql);
					//--ff---通过申请联系方式查看
					$sql_ck = "select 1 from ".table('company_apply_ck')." WHERE company_uid = {$_SESSION['uid']} AND resume_id='{$id}' and company_apply = 1 LIMIT 1";
					$info_ck=$db->getone($sql_ck);
					//--
					if (!empty($info) || !empty($info_ck))
					{
					$show=true;
					}
					elseif($company_info['ck_num'] >0 && $company_info['ck_end_time'] >=time())
					{
						
						$num=$company_info['ck_num']-1;
						$num=$num > 0?$num:0;
						//----
						//----
						$sql="update ".table("company_profile")." set ck_num = ".$num." where uid = {$_SESSION['uid']}";
						$db->query($sql);
						$show=true;
						//exit;
					}
					...
					$show=false;
					//$html="<div class=\"btnIsLogin\"><a class=\"download\" href=\"javascript:void(0);\">查看联系方式</a></div>";
					$html="<div class=\"btnIsLogin\"><a id=\"apply_ck\" href=\"javascript:void(0);\">申请查看<a></div><div style='color:red;' align='center'>仅该会员投递您发布的职位时或者个人会员同意你的申请才可以查看</div>";
					}
				}
				else
				{
				$show=false;
				//$html="<div class=\"btnIsLogin\"><a class=\"download\" href=\"javascript:void(0);\">查看联系方式</a></div>";
				$html="<div class=\"btnIsLogin\"><a id=\"toudi\" href=\"javascript:void(0);\">查看联系方式</a></div>";
				}
			}
		}
...
elseif($act == 'course_contact')
{
	...
	if ($id>0)
	{
		...
		//登录后，查看权限
			...
			//$show=false;
			$show=true;
			//$html="<p>请 <a href=\"".url_rewrite('QS_login',array('url'=>$_SERVER['HTTP_REFERER']),false,NULL,false)."\">[登录]</a> 后查看联系方式，如果您还没有帐号，请先 <a href=\"".$_CFG['main_domain']."user/user_reg.php\">[免费注册]</a></p><p class='train_contact_yidagong'>联系我时，请说在壹打工网看到的，谢谢！</p>";
			}
		}
		if ($show)
		{
		...
		$val=$db->getone($sql);
			...
			$ull="</ul>";
			$html=$ul.$contact.$telephone.$email."<li class='train_contact_yidagong'>联系我时，请说在壹打工网看到的，谢谢！</li>".$ull;
			}
			else
			{
			$ul="<ul>";
...
elseif($act == 'train_contact')
{
	...
	if ($id>0)
	{
		...
		//需要注意
			...
			//$show=false;
			$show=true;
			//$html="<p>请 <a href=\"".url_rewrite('QS_login',array('url'=>$_SERVER['HTTP_REFERER']),false,NULL,false)."\">[登录]</a>  后查看联系方式，如果您没有您没有帐号，请先 <a href=\"".$_CFG['main_domain']."user/user_reg.php\">[免费注册]</a><p class='train_contact_yidagong'>联系我时，请说在壹打工网看到的，谢谢！</p></div>";
			}
		}
		if ($show)
		{
		...
		$val=$db->getone($sql);
			...
			$ull="</ul>";
			$html=$ul.$contact.$telephone.$email.$address.$website."<li class='train_contact_yidagong'>联系我时，请说在壹打工网看到的，谢谢！</li>".$ull;
			}
			else
			{
			$ul="<ul>";
...
//--审核员发布的行业名企
//高级职位
elseif($act == 'shenhejobs_contact')
{
	$id=intval($_GET['id']);
	if ($id>0)
	{
		$show=false;
		if($_CFG['showshenhejobcontact']=='0')
		{
		$show=true;
		}
		elseif($_CFG['showshenhejobcontact']=='1')
		{
			if ($_SESSION['uid'] && $_SESSION['username'] && $_SESSION['utype']=='2')
			{
			$show=true;
		...
		//是否创建高级经理人简历
		elseif($_CFG['showshenhejobcontact']=='2')
		{
			if ($_SESSION['uid'] && $_SESSION['username'] && $_SESSION['utype']=='2')
			{
				$val=$db->getone("select uid from ".table('manager_resume')." where uid='{$_SESSION['uid']}' and complete=1 and audit=1 LIMIT 1");
		...
		if ($show)
		{
		$sql = "select telephone,telephone_show,email,email_show,address,address_show,contact,contact_show,qq,qq_show from ".table('shenhe_jobs')." where id='{$id}' LIMIT 1";
		$val=$db->getone($sql);
			if ($_CFG['contact_img_shenhejob']=='2')
			{
			$token=md5($val['contact'].$id.$val['telephone']);
			$contact=$val['contact_show']=='1'?"<p>联 系 人：<img src=\"{$_CFG['main_domain']}plus/contact_img.php?act=hunterjobs_contact&type=1&id={$id}&token={$token}\"  border=\"0\" align=\"absmiddle\"/></p>":"<p>联 系 人：企业设置不对外公开</p>";
			$telephone=$val['telephone_show']=='1'?"<p>联 系 电 话：<img src=\"{$_CFG['main_domain']}plus/contact_img.php?act=hunterjobs_contact&type=2&id={$id}&token={$token}\"  border=\"0\" align=\"absmiddle\"/></p>":"<p>联 系 电 话：企业设置不对外公开</p>";
