...
elseif($act == 'selectcsvip'){
	...
	if($dqid==0){
		...
		$page=new Page($count['num'], $pnum, $url);
		$sql='select vip_zt.id,vip_zt.uid,vip_user.qid,vip_user.huiyuan,vip_zt.subsite_id,username,title,xs_user,activation,type,duration,bout,cs_ks_time,cs_end_time,add_time from vip_user right join vip_zt on vip_zt.uid = vip_user.id where type="0" '.$where_sql.' ORDER BY vip_zt.id desc limit '.$page->limit;
		$data=$db->getall($sql);
		$smarty->assign("fpage", $page->fpage());
