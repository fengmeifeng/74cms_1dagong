...
require_once(ADMIN_ROOT_PATH.'include/admin_hymq_fun.php');
...
if($act == 'jobs')
{
	...
	$getsql="SELECT * FROM ".table('shenhe_jobs').$wheresql.$oederbysql;
	...
	get_token();
	$smarty->display('hymq/admin_hunter_jobs.htm');
}
...
elseif($act == 'bm_hymq')
{
	...
	$hunterjobs = get_hunterjobs($offset,$perpage,$getsql);
	$smarty->assign('pageheader',"报名行业名企管理");
	$smarty->assign('hunterjobs',$hunterjobs);
	$smarty->assign('now',time());
	$smarty->assign('total',$total);
	$smarty->assign('page',$page->show(3));
	...
	$smarty->display('hymq/bm_hymq.htm');
}
elseif($act == 'del_bm_hymq')
{	
	check_token();
	//check_permissions($_SESSION['admin_purview'],"hun_user_del");
	$tuid =!empty($_REQUEST['id'])?$_REQUEST['id']:adminmsg("你没有选择记录！",1);

	!delete_bm_hymq($tuid)?adminmsg("删除失败！",0):adminmsg("删除成功！",2);;
...
elseif($act == 'add_jobs')
{
	...
	$smarty->assign('user',$user);
			...
			$smarty->assign('title','发布行业名企 - 审核员中心 - '.$_CFG['site_name']);
	...
	$smarty->display('hymq/admin_hunter_jobs_add.htm');
}
elseif ($act=='addjobs_save')
{
	...
	$days=intval($_POST['days']);
				...
				{
				showmsg("有效时间最少为 ".$_CFG['shenhe_add_days_min']." 天！",1);
				}
	if ($_CFG['operation_shenhe_mode']=='1')
	{
					...
					showmsg("你的".$_CFG['shenhe_points_byname']."不足，请充值后再发布！",0,$link);
					}
	...
	//--公司所在城市
	$setsqlarr['gs_district']=!empty($_POST['gs_district'])?intval($_POST['gs_district']):showmsg('请选择公司所在城市！',1);
	...
	//$setsqlarr['jobs_name']=!empty($_POST['jobs_name'])?trim($_POST['jobs_name']):showmsg('您没有填写职位名称！',1);//==职位名称判断
	...
	$setsqlarr['jiezhan']=!empty($_POST['jiezhan'])?trim($_POST['jiezhan']):showmsg('您没有填写接站！',1);
	...
	//echo $setsqlarr['sex']."<br>";echo $setsqlarr['sex_cn'];exit;
	//---ffff
	...
	if ($_CFG['operation_shenhe_mode']=='1')
	{
		...
		write_memberslog($_SESSION['uid'],3,9201,$_SESSION['username'],"发布了职位：<strong>{$setsqlarr['jobs_name']}</strong>，({$operator}{$points_rule['shenhe_shenhejobs_add']['value']})，(剩余:{$user_points})");
		}
...
elseif($act=='addjobs_save_succeed'){
	...
	$smarty->display('hymq/hymq_addjobs_succeed.htm');
}
//---ff
elseif($act == 'edit_jobs')
{
	...
	$smarty->assign('jobs',$jobs);
	$smarty->assign('subsite',get_subsite_list());
	$smarty->display('hymq/admin_hunter_jobs_edit.htm');
}
elseif ($act=='editjobs_save')
{
	...
	$setsqlarr['wage']=!empty($_POST['wage'])?intval($_POST['wage']):adminmsg('请选择薪资待遇！',1);
	$setsqlarr['wage_cn']=trim($_POST['wage_cn']);
	$setsqlarr['wage_structure']=!empty($_POST['wage_structure'])?$_POST['wage_structure']:adminmsg('请选择薪资！',1);
	$setsqlarr['socialbenefits']=trim($_POST['socialbenefits']);
	$setsqlarr['livebenefits']=trim($_POST['livebenefits']);
	$setsqlarr['annualleave']=trim($_POST['annualleave']);
	...
	$wheresql=" id='".$id."' ";
	if (!updatetable(table('shenhe_jobs'),$setsqlarr,$wheresql)) adminmsg("保存失败！",0);
	$link[0]['text'] = "返回职位列表";
...
elseif($act == 'hunter_list')
{
	...
	$smarty->assign('clist',$clist);
 	$smarty->assign('page',$page->show(3));
	$smarty->display('hymq/admin_hunter_list.htm');
}
...
elseif($act == 'edit_hunter_profile')
{
	...
	$smarty->display('hymq/admin_hunter_profile_edit.htm');
}
...
elseif($act == 'members_list')
{
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('hymq/admin_hunter_user_list.htm');
}
...
elseif($act == 'members_add')
{
	...
	$smarty->assign('points',get_cache('points_rule'));
	$smarty->display('hymq/admin_hunter_user_add.htm');
}
...
elseif($act == 'user_edit')
{
	...
	$smarty->assign('givesetmeal',get_setmeal(false));
	$smarty->assign('url',$_SERVER["HTTP_REFERER"]);
	$smarty->display('hymq/admin_hunter_user_edit.htm');
}
	...
	if ($hunter_uid)
	{
 		...
 		//会员套餐变更记录。管理员后台修改会员套餐：修改会员。3表示：管理员后台修改
 			...
 			if ($setsqlarr['endtime']<>"")
			{
				...
				if (!updatetable(table('shenhe_jobs'),$setmeal_deadline," uid='{$hunter_uid}' AND add_mode='2' "))adminmsg('修改出错！',0);
			}
	}
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = $_POST['url'];
...
elseif($act == 'meal_log')
{
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('hymq/admin_hunter_meal_log.htm');
}
...
elseif($act == 'meal_members')
{
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('hymq/admin_hunter_meal_members.htm');
}
...
{	
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('hymq/admin_order_list.htm');
}
elseif($act == 'order_set')
{
	...
	$smarty->display('hymq/admin_order_set.htm');
}
...
elseif($act == 'show_order')
{
	...
	$smarty->assign('url',$_SERVER["HTTP_REFERER"]);
	$smarty->assign('payment',get_order_one($_GET['id']));
	$smarty->display('hymq/admin_order_show.htm');
}
 ...
 elseif($act == 'meal_log_pie')
{
	...
	$smarty->display('hymq/admin_hunter_meal_log_pie.htm');
}
