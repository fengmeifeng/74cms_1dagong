...
if($act == 'jobs')
{
	...
	else
	{
			...
			{
				$wheresqlarr['subsite_id']=intval($_CFG['subsite_id']);
			}
	...
	if(intval($_CFG['subsite_id'])!="0")
	{
		isset($wheresql)?$wheresql.=' and subsite_id='.intval($_CFG['subsite_id']):$wheresql =" where subsite_id=".intval($_CFG['subsite_id']);
	}
	///---fffffff
	//echo $wheresql;exit;
	if ($tablename=="all")
	...
	get_token();
	$smarty->display('company_shenhe/admin_company_jobs.htm');
}
...
elseif($act == 'edit_jobs')
{
	...
	$smarty->display('company_shenhe/admin_company_jobs_edit.htm');
}
...
elseif($act == 'company_list')
{
	...
	if ($key && $key_type>0)
	{
		...
		//if($key_type===1)$wheresql=" WHERE c.companyname = '{$key}'";
		...
		elseif ($key_type===7)$wheresql=" WHERE m.xs_user  like '%{$key}%'";		
		$oederbysql="";
	}
	//echo $wheresql;exit;
	//-------fffff
	if(intval($_CFG['subsite_id'])!="0")
	{
		...
		empty($wheresql)?$wheresql=" where m.subsite_id=".intval($_CFG['subsite_id']):$wheresql=$wheresql.' and m.subsite_id='.intval($_CFG['subsite_id']);
	}
	///---fffffff
	//echo $wheresql;exit;
	$_GET['audit']<>""? $wheresqlarr['c.audit']=intval($_GET['audit']):'';
	$_GET['yellowpages']<>""? $wheresqlarr['c.yellowpages']=intval($_GET['yellowpages']):'';
	//intval($_CFG['subsite_id'])!="0"?$wheresqlarr['c.subsite_id']=intval($_CFG['subsite_id']):'';
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_company_list.htm');
}

elseif($act == 'company_perform')/*删除公司相关信息*/
{
	check_token();
...
elseif($act == 'edit_company_profile')
{
	...
	$smarty->display('company_shenhe/admin_company_profile_edit.htm');
}
...
{	
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_order_list.htm');
}
elseif($act == 'show_order')
{
	...
	$smarty->display('company_shenhe/admin_order_show.htm');
}
...
elseif($act == 'order_set')
{
	...
	$smarty->assign('url',$_SERVER["HTTP_REFERER"]);
	$smarty->assign('payment',get_order_one($_GET['id']));
	$smarty->display('company_shenhe/admin_order_set.htm');
}
...
elseif($act == 'meal_members')
{
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_company_meal_members.htm');
}
elseif($act == 'meal_log')
{
	get_token();
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_company_meal_log.htm');
}
elseif($act == 'meal_log_pie')
{
	...
	$smarty->display('company_shenhe/admin_company_meal_log_pie.htm');
}
...
elseif($act == 'members_list')
{
	...
	if ($key && $key_type>0)
	{
		...
		elseif ($key_type===5)$wheresql.=" AND c.companyname like '%{$key}%'";
		elseif ($key_type===6)$wheresql.=" AND m.xs_user like '%{$key}%'";
		$oederbysql="";
	}
	else
	{	
	...
	if(intval($_CFG['subsite_id'])!="0")
	{
		...
		$wheresql.=" and  m.subsite_id=".intval($_CFG['subsite_id']);
	}
	///---fffffff
	...
	$smarty->assign('member',$member);
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_company_user_list.htm');
}
...
elseif($act == 'members_add')
{
	...
	$smarty->display('company_shenhe/admin_company_user_add.htm');
}
elseif($act == 'members_add_save')
{
	...
	$sql['password'] = md5(trim($sql['password']));
	$sql['reg_time']=time();
...
{	
	...
	$smarty->display('company_shenhe/admin_consultant_install.htm');
}
//保存  设置顾问
elseif($act == 'consultant_install_save')
{
	//得到 顾问的id 
	$id = !empty($_GET['id'])?intval($_GET['id']):adminmsg("选择顾问发生错误！",1);
	//得到要设置顾问的企业会员uid 
	$tuid =!empty($_REQUEST['tuid'])?$_REQUEST['tuid']:adminmsg("你没有选择会员！",1);
	$tuid=explode(",", $tuid);
	foreach ($tuid as $uid) {
		updatetable(table('members'),array('consultant' => $id )," uid='{$uid}'");
	}
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = "?act=members_list";
	adminmsg('设置成功！',1,$link);
}
...
$company_user['ck_num']=$company_profile['ck_num'];
$company_user['ck_end_time']=$company_profile['ck_end_time'];
	...
	$smarty->assign('givesetmeal',get_setmeal(false));
	$smarty->assign('url',$_SERVER["HTTP_REFERER"]);
	$smarty->display('company_shenhe/admin_company_user_edit.htm');
}
...
elseif($act == 'ck_resume_edit')
{
	//echo "设置查看次数";exit;
	check_token();
	check_permissions($_SESSION['admin_purview'],"com_user_edit");
	if (intval($_POST['ck_num'])<0) adminmsg('请输入查看次数！',1);
	if (trim($_POST['ck_end_time'])=='') adminmsg('请选择到期时间！',1);
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = $_POST['url'];
	$user=get_user($_POST['company_uid']);
	$ck_num=intval($_POST['ck_num']);
	$ck_end_time=strtotime($_POST['ck_end_time']);	
	//echo "<pre>";print_r($user);echo $ck_end_time;echo $t;exit;
	update_ck_num($user['uid'],$ck_num,$ck_end_time);
	
	write_memberslog(intval($_POST['company_uid']),1,3203,$user['username'],"{$_SESSION['admin_name']},操作修改查看简历次数，(剩余查看次数:{$ck_num})，到期时间：".$_POST['ck_end_time'],1,1012,"管理员操作查看简历次数","{$_POST['ck_num']}","{$ck_end_time}");
		...
		$notes="操作人：{$_SESSION['admin_name']},说明：修改会员 {$user['username']} 查看次数 ({$t}{$_POST['ck_num']})。到期时间：{$ck_end_time}";
		write_setmeallog($_POST['company_uid'],$user['username'],$notes,3,$amount,$ismoney,1,1);
		*/
...
elseif($act == 'xs_user_edit')//--修改客户经理
{
	
	check_token();
	check_permissions($_SESSION['admin_purview'],"com_user_edit");
	if(set_user_xs_user(trim($_POST['xs_user']),intval($_POST['xs_user_uid'])))
	{
		$link[0]['text'] = "返回列表";
		$link[0]['href'] = $_POST['url'];
		adminmsg('操作成功！',2,$link);
...
elseif($act == 'userstatus_edit')//--修改状态
{
	check_token();
	check_permissions($_SESSION['admin_purview'],"com_user_edit");
...
elseif($act == 'comment_list')
{
	...
	$smarty->assign('clist',$clist);
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_comment_list.htm');
}
...
elseif($act == 'company_img')
{
	...
	$smarty->assign('clist',$clist);
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_company_img.htm');
}
...
elseif($act == 'company_news')
{
	...
	$smarty->assign('clist',$clist);
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_company_news.htm');
}
...
elseif($act == 'edit_company_news')
{
	...
	$smarty->assign('url',$_SERVER["HTTP_REFERER"]);
	$smarty->assign('pageheader',"企业新闻");
	$smarty->display('company_shenhe/admin_company_news_edit.htm');
}
...
elseif($act == 'consultant')
{
	...
	$smarty->assign('clist',$clist);
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_consultant_list.htm');
}
//顾问  管理
elseif($act == 'consultant_manage')
{
	...
	$smarty->assign('page',$page->show(3));
	$smarty->display('company_shenhe/admin_consultant_manage.htm');
}
//管理 顾问  重置 按钮 
elseif($act == 'resetting')
{
	...
	//对这些会员进行重置顾问
	if(updatetable(table('members'),array('consultant'=>0)," uid in ({$member_del_id}) ")){
		adminmsg('重置成功!',1);
	}else{
		adminmsg('重置过程发生失败!',1);
	}
...
elseif($act == 'consultant_add')
{
	...
	$smarty->assign('pageheader',"顾问管理");
	$smarty->display('company_shenhe/admin_consultant_add.htm');
}
...
elseif($act == 'consultant_edit')
{
	...
	$smarty->assign('consultant',$consultant);
	$smarty->assign('pageheader',"顾问管理");
	$smarty->display('company_shenhe/admin_consultant_edit.htm');
}
