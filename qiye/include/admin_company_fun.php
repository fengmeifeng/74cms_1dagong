 ...
 function get_company($offset,$perpage,$get_sql= '',$mode=1)
{
	...
	$row_arr = array();
	$limit=" LIMIT ".$offset.','.$perpage;
	$result = $db->query("SELECT c.*,m.username,m.xs_user,m.reg_time,m.mobile,m.email as memail,{$colum} FROM ".table('company_profile')." AS c ".$get_sql.$limit);
	while($row = $db->fetch_array($result))
	{
...
function get_company_one_id($id)
{
	...
	$sql = "select c.*,m.xs_user from ".table('company_profile')." as c left join ".table('members')." as m on c.uid=m.uid where c.id='{$id}'";//-fff搜索出客户经理
	$val=$db->getone($sql);
	$val['user']=get_user($val['uid']);
	return $val;
}
...
function set_user_xs_user($xs_user,$uid)
{
	global $db;
	$xs_user=$xs_user;
	$uid=intval($uid);
	//echo $xs_user."<br>";echo $uid;exit;
	if (!$db->query("UPDATE ".table('members')." SET xs_user= '{$xs_user}' WHERE uid={$uid} LIMIT 1")){ return false;}
	else{
	return true;}
}
