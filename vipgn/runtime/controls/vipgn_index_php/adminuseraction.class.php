<?php
	class AdminuserAction extends Common {
		//权限用户列表
		function index(){
		...
		function modmima(){
			if(!empty($_GET['name'])){
				$data=D("vip_adminpassword")->where(array("dqname"=>iconv("UTF-8", "GBK", urldecode($_GET['name']))))->select();
				$this->assign("data",$data[0]);
				$this->display(mod);
			}
		}
		...
		function updata(){
			if(!empty($_POST)){
				...
				if(!empty($id)){
						$this->success("成功!", 2, "adminuser/modmima/name/".$_POST['dqname']);
					}else{
						$this->error("失败!",2,"adminuser/modmima/name/".$_POST['dqname']);
					}

				}else{
					$this->error("权限密码两次输入不相等,请重新输入",2,"adminuser/modmima/name/".$_POST['dqname']);
				}
			}
		}
