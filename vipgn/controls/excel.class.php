...
/* 		$cfg_dbhost = '192.168.3.246';
		...
		// var_dump($data);exit;
			...
			foreach ($data as $k=>$v) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue("S".$k, $this->convertUTF8($v['km1']));
				$objPHPExcel->getActiveSheet()->setCellValue("T".$k, $this->convertUTF8($v['km2']));
			}
			//直接输出到浏览器
...
/* 		$cfg_dbhost = '192.168.3.246';
		...
		// var_dump($data);exit;
			...
			$objPHPExcel->getActiveSheet()->getColumnDimension('T')->setWidth(45);
			...
			foreach ($data as $k=>$v) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue("N".$k, $this->convertUTF8($v['birthday']));
				//---FFFF
				$objPHPExcel->getActiveSheet()->setCellValue("O".$k, $this->convertUTF8($v['kch']));
				$objPHPExcel->getActiveSheet()->setCellValue("P".$k, $this->convertUTF8($v['zwh']));
				$objPHPExcel->getActiveSheet()->setCellValue("Q".$k, $this->convertUTF8($v['ksbh']));
				//---FFFF
				$objPHPExcel->getActiveSheet()->setCellValue('R'.$k, $this->convertUTF8("点击查看正面"));
				$objPHPExcel->getActiveSheet()->getCell('R'.$k)->getHyperlink()->setUrl("http://www.1dagong.com/zt/hefeizhaomu/".$v['identity_up']);

				$objPHPExcel->getActiveSheet()->setCellValue('S'.$k, $this->convertUTF8("点击查看反面"));
				$objPHPExcel->getActiveSheet()->getCell('S'.$k)->getHyperlink()->setUrl("http://www.1dagong.com/zt/hefeizhaomu/".$v['identity_down']);
				...
				$objPHPExcel->getActiveSheet()->setCellValue('T'.$k, $this->convertUTF8("点击查看证件照"));
				$objPHPExcel->getActiveSheet()->getCell('T'.$k)->getHyperlink()->setUrl("http://www.1dagong.com/zt/hefeizhaomu/".$v['photo']);
				...
				$objPHPExcel->getActiveSheet()->setCellValue("U".$k, $this->convertUTF8($v['km1']));
				$objPHPExcel->getActiveSheet()->setCellValue("V".$k, $this->convertUTF8($v['km2']));
				$objPHPExcel->getActiveSheet()->setCellValue("W".$k, $this->convertUTF8($v['mscj']));
