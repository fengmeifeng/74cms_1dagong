...
<tr class="khbox tab" id="dengji<{$item.id}>" >
	...
	<td noWrap  align='left'> <{if $item.f_bianghao!='0'}> <b style="color:#565656;"><{$item.f_bianghao}></b> <{/if}> </td>
	<td noWrap  align='center'> <{$item.guanxi}> </td>
	<td noWrap  align='right'> <{$item.q}>元 </td>
    ...
    <td> <a href="<{$url}>/huiyuaninfo/id/<{$item.id}>">查资料</a> </td>
</tr>
<script type="text/javascript">
	var m=<{$item.m}>;
	if(m > 1){
