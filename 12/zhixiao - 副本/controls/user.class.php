		...
		function guo60day(){
			...
			switch($_SESSION['priv']){
				case $_SESSION['priv']==2 || $_SESSION['priv']==4:
					$user=D("tuijian")->field("id")->where(array("bianhao"=>$_SESSION['user']))->find();
					if(!empty($_GET['search'])){
						$where_sql=array("jihuo"=>'1',"status"=>'2',"path"=>"%,".$user['id'].",%",$_GET['search']=>'%'.$_GET['findvalue'].'%');
					}else{
						$where_sql=array("jihuo"=>'1',"status"=>'2',"path"=>"%,".$user['id'].",%");
					}
					break;
				case 1:
					if(!empty($_GET['search'])){
						$where_sql=array("jihuo"=>'1',"status"=>'2',$_GET['search']=>'%'.$_GET['findvalue'].'%');
					}else{
						$where_sql=array("jihuo"=>'1',"status"=>'2' );
					}
					break;
				default:	//默认无
					exit('没权访问！');
			...
			if(!empty($_POST)){
				...
				if($_SESSION['name']!=''){
					if($_POST['ruzhi_time']!='' && $_POST['ruzhi_name']!=''){
						$data=D("tuijian")->field("bianhao,name,sphone,pid,f_bianghao,path")->where(array("id"=>$_POST['id']))->find();	//查询要入职人员信息
						if($data['pid'] <=1){
...
/*------------------------------------------------------------------------------------------------------------------------------------------*/
		...
		function jihuohuiyuan(){
			...
			//-------------------------------
				if(!empty($_GET['id'])){
					$user=D("tuijian")->field("id,bianhao,name,sphone,jihuo,status,ruzhi_time,f_bianghao,pid,path,jibie")->where(array("id"=>$_GET['id']))->find();
					//未入职不可以激活
					if($user['status']!=1){
						...
						foreach ($path as $k => $v){
							if($v!='' && strval($v)!='0'){
								...
								if($m < 13){						//超过12层就执行
									...
									//---奖金增加的日志--and------------------------
									$urdat=D("tuijian")->field("id,bianhao,name,sphone,status,jiangjin,f_bianghao,pid,path")->where(array("id"=>$v))->find();
									if($urdat['status']=='1'){		//当前用户在职才能拿到推荐奖金到日志!
									...
									//---奖金增加的日志--end------------------------
									//当前用户在职才能拿到推荐奖金!
									$upa=D("tuijian")->where(array("id"=>$v,'status'=>'1'))->update("jiangjin=jiangjin+".$q);
			...
			if(!empty($_POST)){
				...
				if($_SESSION['name']!=''){
					...
					if($id > 0){
						...
						D("caozuolog")->insert($caozuo);
						//--记录日志---end----------------------------
						$this->success('用户资料修改成功！', 1 ,$_GET['dqurl']);
					}else{
						$this->error('用户资料修改失败！', 1 , $_GET['dqurl']);
					}
				}
				//---------------------
				}else{
...
/*------------------------------------------------------------------------------------------------------------------------------------------*/
		...
		function lizhi(){
			...
			if(!empty($_POST)){
				if($_POST['lizhi_time']!=''){
					$data=D("tuijian")->field("bianhao,name,pid,f_bianghao,path,ruzhi_time")->where(array("id"=>$_POST['id']))->find();		//查询要入职人员信息
					if($data['pid'] <=1){
		...
		function ruzhi(){
			if(!empty($_POST)){
				if($_POST['ruzhi_time']!=''){
					$data=D("tuijian")->field("bianhao,name,pid,f_bianghao,path")->where(array("id"=>$_POST['id']))->find();	//查询要入职人员信息
					if($data['pid'] <=1){
						$this->error('该用户不能入职！', 1, "jihuiuser");		//报单中心 人员不可以入职
					}
					if($_SESSION['name']!=''){
						$_POST['status']='1';									//入职状态为1 -> "入职"
						$_POST['ruzhi_time']=strtotime($_POST['ruzhi_time']);	//入职时间
						...
						if($id > 0){		//执行成功！
							//--记录日志---and--------------------------------
							$caozuo=array("name"=>$_SESSION['name'],"ip"=>getip(),"time"=>time(),"caozuo"=>'入职会员  编号：'.$data['bianhao'].'， 姓名：'.$data['name'].', 入职时间：'.date("Y-m-d",$_POST['ruzhi_time']));
							D("caozuolog")->insert($caozuo);
							//--记录日志---end----------------------------
							$this->success('用户入职成功！', 1, "jihuiuser");
						}else{
							$this->error('用户入职失败！', 1, "jihuiuser");
						}
					}
				}else{
					$this->error('入职时间不能为空！', 1, "jihuiuser");
				}
			}
		}*/
		//---------------
		...
		function daochuexcel(){
			...
			//-------end----------------------------------
			$page=new page(D("tuijian")->where($where_sql)->total(),20);
			$data=D("tuijian")->where($where_sql)->limit($page->limit)->order("jihuo_time asc")->select();
			$jibei=D("jibie")->select();
			foreach ($data as $k => $v){
				foreach ($jibei as $i => $s){
					if($s['id']==$v['jibie']){
