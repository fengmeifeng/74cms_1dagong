...
<tr>
	<td colspan="21" align="left">
		&nbsp;查找：<form action="<{$url}>/zaizhi" method="get" style="display:inline;">
...
<tr class="khbox tab" onDblClick="location.href='<{$url}>/upuser/id/<{$item.id}>'">
	...
	<td noWrap  align='center'> <{if $item.status=='1'}> <font color="green">已入职</font> <{else}> <font color="red">未入职</font> <{/if}>		<{if $item.ruzhi_time!='0'}> <{$item.ruzhi_time|date_format:"%Y-%m-%d"}> <{/if}> </td>
