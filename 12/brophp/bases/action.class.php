 ...
 * ******************************************************************************/
	class Action extends MyTpl{
		 ...
		 * @param	string	$location	设置跳转的新位置
		 */
		function errorbb($mess="操作失败", $timeout=3, $location=""){
			$this->assign("mess", $mess);
			$this->assign("timeout", $timeout);
			$this->assign("location", "window.location='".$location."'");
			$this->assign("mark", false); //如果失败 $mark=false
			$this->display("public/success");
			exit;
		}
		...
		function success2($mess="操作成功", $timeout=1, $location=""){
			$this->pub2($mess, $timeout, $location);
			$this->assign("mark", true);  //如果成功 $mark=true
			$this->display("public/success");
			exit;
		}
		
		function error2($mess="操作失败", $timeout=3, $location=""){
			$this->pub($mess, $timeout, $location);
			$this->assign("mark", false); //如果失败 $mark=false
			$this->display("public/success");
			exit;
		...
		private function pub2($mess, $timeout, $location){	
			$this->caching=0;     //设置缓存关闭
			if($location==""){
