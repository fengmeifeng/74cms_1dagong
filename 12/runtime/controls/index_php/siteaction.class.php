<?php
	class SiteAction extends Common {
		function index(){
		...
		//父类菜单显示
		function flcd(){
			$data=D("menu_ht")->where(array("pid"=>"0"))->select();
			//p($data);
		...
		//菜单显示
		function menu(){
			$sql = "select * from menu_ht order by concat(path,id)";
			$data=D("menu_ht")->query($sql,"select"); 
            //-----------------------------------
            foreach ($data as $i=>$v){
		...
		function add_zmenu(){
			if(!empty($_GET['pid'])){
				$data=D("menu_ht")->where(array("id"=>$_GET['pid']))->select();
		...
		function modf(){
			if(!empty($_GET["id"])){
				$data=D("menu_ht")->where(array("id"=>$_GET['id']))->select();
	            $this->assign("data",$data[0]);
	            $this->display();
			}
			//销售变量
		...
		function updata(){
			...
			if(!empty($_POST)){
				$id=D("menu_ht")->where(array("id"=>$_POST["id"]))->update();
				if(!empty($id)){
		...
		function del(){
			if(!empty($_GET["id"])){
				$id=D("menu_ht")->delete(array("id"=>$_GET["id"]),array("path"=>"%,".$_GET["id"].",%"));
				if(!empty($id)){
