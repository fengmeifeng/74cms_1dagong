...
<script type="text/javascript">
	...
	function statusl(a,b){
		if(b=='lz'){
			...
			$("#formshijian").attr("action","<{$app}>/user/lizhi");
		}
...
<tr>
	<td colspan="21" align="left">
		&nbsp;查找：<form action="<{$url}>/add_gs_jihuiuser" method="get" style="display:inline;">
			...
			<input type="button" value=" 导出 " class="sub" onclick="location='<{$app}>/user/daochuexcel?<{$changshu}>';">
		</form>
	</td>
</tr>
<tr class="biaoti">
	...
	<td noWrap width="65" onDblClick="location.href='<{$url}>/add_gs_jihuiuser/paixu/jiangjin/fs/<{$fs}>' " style="cursor:pointer" >奖金</td>
...
<tr class="khbox tab" >
	...
	<td noWrap  align='center'> <a href="<{$app}>/user/huiyuaninfo/id/<{$item.id}>/dqurl/jihuiuser">查资料</a> <a href="<{$app}>/user/huiyuantuijian/id/<{$item.id}> "> 推荐列表</a>  
		 <{if $item.status=='1'}>
		 ...
		 <div class="status statusxianshil<{$item.id}>" style='display:none;margin: 4px -25px;'>  
			<div class="content"> 
				<form method="post" action="<{$app}>/user/lizhi" id="formshijian">
		...
		<div class="status statusxianshir<{$item.id}>" style='display:none;margin: 4px -25px;'>  
			<div class="content" style="height:60px;line-height:25px; padding-top:15px;"> 
				<form method="post" action="<{$app}>/user/ruzhihuiyuan" id="formshijian">
					<input type="hidden" name="id" value="<{$item.id}>" />
