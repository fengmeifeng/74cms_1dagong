...
require_once(QISHI_ROOT_PATH.'include/mysql.class.php');
	...
	//----获取总数
	$total_sql="SELECT COUNT(*) AS num FROM ".table('personal_favorites')." as p left join ".table('jobs')." as j ON p.jobs_id=j.id where p.personal_uid={$userid}";
	...
	//----fff
	//echo $offset;exit;
	$res = $db->query("SELECT p.*,j.companyname,j.district_cn,j.wage_cn,j.company_id,j.uid FROM ".table('personal_favorites')." as p left join ".table('jobs')." as j  ON p.jobs_id=j.id WHERE p.personal_uid='{$userid}' LIMIT {$offset},{$pagesize}");
	while($row = $db->fetch_array($res))
		{
			$row['addtime']=daterange(time(),$row['addtime'],'Y-m-d',"#FF3300");
			if($row['wage_cn'] !='面议'){$row['wage_cn']=$row['wage_cn']."元/月";}
			if($row['company_id'] && $row['companyname'])
			{
				$list[] = $row;
			}
		}
