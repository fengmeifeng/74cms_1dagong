...
function del_jobs_apply($del_id,$uid)
{
	...
	write_memberslog($uid,2,1302,$_SESSION['username'],"删除了职位申请($sqlin)");
	return true;
}
function p_jobs_apply($id,$uid)
{
	global $db;
	
	$uidsql=" personal_uid=".intval($uid)." ";
	if (!is_array($id)) $id=array($id);
	$sqlin=implode(",",$id);
	if (!preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin)) return false;
	echo "select did from ".table('personal_jobs_apply')." WHERE did IN (".$sqlin.") ".$uidsql."";exit;
	$res = $db->getall("select did from ".table('personal_jobs_apply')." WHERE ".$uidsql."");
	if($res)
	{
		return true;
	}else{
...
$db = new mysql($dbhost,$dbuser,$dbpass,$dbname);
	...
	//echo del_jobs_apply($did,$userid);exit;
	//echo p_jobs_apply($did,$userid);exit;
	if(!p_jobs_apply($did,$userid)){
	$result['code']=-1;
